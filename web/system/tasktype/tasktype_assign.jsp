	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#taskTypeDg").datagrid({
				...
				pagination:true,
				fit:true,
				toolbar:'#taskTypeTool',
				singleSelect:false,
				rownumbers:true,
				rowrap:true,
				striped:true,
				pageList:[50,100,200],
				url:'sysTaskTypeAssign/datagrid',
				idField:'ID',
				onLoadSuccess:function(data) {
					if(targetOperId!=null) {
						$.ajax({
							url:'sysTaskTypeAssign/getSysTaskTypeAssignResult?targetOperId=' + targetOperId,
							method:'post',
							dataType:'json',
							success:function(rs) {
								...
								if(statusCode == 'success') {         //返回的数据不为空时，才进行勾选当前操作员的角色
									...
									for(var i=0;i<callerid_ids.length;i++) {
										$("#taskTypeDg").datagrid('selectRecord',callerid_ids[i]);
									}
								}
							}
						});
			...
			$("#taskTypeAssignDlg").dialog({
				onClose:function() {
					targetOperId=null;
					$("#taskTypeDg").datagrid("clearSelections");
				}
			});
			
			$("#addTaskTypeDlg").dialog({
    			onClose:function() {
    				$("#taskTypeForm").form('clear');
    			}
    		});
		});
		...
		function rowformater(value,data,index) {
			return "<a href='#' onclick='javascript:taskTypeAssign(\"" + data.OPER_ID +"\",\""+ data.OPER_NAME + "\")'><img src='themes/icons/dial.png' border='0'>任务类型分配</a>";
		}
		...
		//主叫号码分配
		function taskTypeAssign(operId,operName) {
			
			targetOperId = operId;
			
			$("#taskTypeDg").datagrid("load",{
				targetOperId:targetOperId
			});
			
			$("#taskTypeAssignDlg").dialog('setTitle',"操作员：" + operName + "(" + operId + ")的任务类型分配").dialog('open');
			
		}
		
		//保存任务类型的分配
		function saveSysTaskTypeAssign() {
			
			ids = getSelectedRows();       //取得选中的 ID
			
			$.ajax({
				url:'sysTaskTypeAssign/saveSysTaskTypeAssign?targetOperId=' + targetOperId + "&ids=" + ids,
				method:'post',
				dataType:'json',
				success:function(rs) {
					var statusCode = rs.statusCode; //返回的结果类型
		...
		function getSelectedRows() {
			...
			var rows = $('#taskTypeDg').datagrid('getSelections');
			var ids = [];
...
<body id="orgBody" style="margin-top:1px;margin-left:1px;">
	...
	<!-- 任务类型分配弹窗 -->
	<div id="taskTypeAssignDlg" class="easyui-dialog" style="width:80%;height:80%;padding:10px 20px;" modal="true" closed="true" buttons="#taskTypeAssignDlgBtn">
		<table id="taskTypeDg">
				<thead>
					<tr style="height:12px;">
						<th data-options="field:'ck',checkbox:true"></th>		
						<th data-options="field:'TASK_TYPE',width:200,align:'center'">任务类型</th>
						<th data-options="field:'NUMBER_ORDER',width:400,align:'center'">任务序号</th>
	...
	<div id="taskTypeAssignDlgBtn">
		<a href="#" id="easyui-add" onclick="saveSysTaskTypeAssign()" class="easyui-linkbutton" iconCls="icon-ok" plain="true">保存任务类型分配</a>
	</div>
	
	<div id="taskTypeTool" style="padding:5px;">
		<a href="#" id="easyui-add" onclick="taskTypeAdd()" class="easyui-linkbutton" iconCls='icon-add' plain="true">新增任务类型</a>
	</div>
	
	<div id="addTaskTypeDlg" class="easyui-dialog" style="width:40%;height:40%;padding:10px 20px;" modal="true" closed="true" buttons="#addTaskTypeDlgBtn">
